...
def generate_mcqs(text, num_questions):
    ...
    for i, sentence in enumerate(sentences[:num_questions]):
        ...
        question = f"What is the meaning of the sentence: '{sentence.strip()}'?"

        # Extract potential keywords (longer words or nouns)
        words = [word for word in sentence.split() if len(word) > 3 and not word.isdigit()]
        
        # If not enough words, fallback to a generic set of options
        if len(words) < 4:
            words = ["Meaning", "Concept", "Definition", "Explanation"]
        
        # Generate random options, ensuring no duplicates
        answer = random.choice(words)
        options = [answer] + random.sample(words, min(3, len(words) - 1))
...
# Number of questions input (with max limit of 50)
num_questions = st.number_input("Enter the number of MCQs you want to generate", min_value=1, max_value=50, step=1)
